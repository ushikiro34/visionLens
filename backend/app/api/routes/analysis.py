...
# 서비스 싱글톤
_yolo = YOLOService()
_fill_calc = FillRatioCalculator()
_rag = HybridFoodSearch()
_calorie_calc = CalorieCalculator()
_hitl_svc = HITLService()
...
async def analyze_image(
    ...
    # 1. Vision 분석
    vision_result = await _yolo.analyze(image)
    ...
    for detected in vision_result.foods:
        ...
        fill_3d, bowl_profile = _fill_calc.calculate_3d(
        ...
        # 4. RAG 검색
        rag_result = await _rag.search(detected.food_name, top_k=5)
    ...
    session = await _hitl_svc.create_session(
        db,
        vision_result={
...
async def apply_correction(
    ...
    try:
        await _hitl_svc.apply_correction(
            db, session_id,
            [c.model_dump() for c in request.corrections],
        )
...
async def approve_session(
    ...
    """
    try:
        session = await _hitl_svc.approve(db, session_id)
        return {
            "status": "ok",
...
async def save_feedback(
    ...
    await _hitl_svc.save_feedback(db, request.session_id, request.feedback_type)
    return {"status": "ok"}
